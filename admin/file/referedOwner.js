async function seenbar(){
    seen=[];
    var val={};
  await firebase.firestore().collection('referedOwner').get().then((res) =>{ 
...
//    console.log(element)
   ...
   tenantName.innerText = doc.ownerName;
   tenantPhone.innerHTML = doc.ownerPhone;
   tenantCity.innerHTML = doc.ownerCity;
...
function updateUnseenVisit(){
    ...
    input.forEach((box)=>{
        if(box.checked==true){
            ...
            console.log(ID)
            var seen = true;
            firebase.firestore().collection('referedOwner').doc(ID).update({
                  seen
            })
        }
    })
